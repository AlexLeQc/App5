...
class markov():
    ...
    def find_author(self, oeuvre):
        ...
        with open(oeuvre, 'r', encoding='utf8') as file:
            textinc = file.read()
            textinc = textinc.lower()
            words = [word for word in textinc.split() if len(word) > 2]

        # Calcul du vecteur de fréquence de l'oeuvre inconnue
        freq_dict_inconnu = {}
        for i in range(len(words)-self.ngram+1):
            ngrammeinc = " ".join(words[i:i+self.ngram])
        ...
        #norme du vecteur de l'inconnue
        norm_inconnu = math.sqrt(sum(freq_dict_inconnu[ngrammeinc]**2 for ngrammeinc in freq_dict_inconnu))

        #calcul avec les autre auteur

        for auteur in self.freq_dict.keys():
            freq_dict_auteur = self.freq_dict[auteur]
            dot_product = sum(freq_dict_auteur.get(ngrammeinc, 0)* freq_dict_inconnu.get(ngrammeinc, 0) for ngrammeinc in set(freq_dict_auteur) | set(freq_dict_inconnu))
            norm_auteur = math.sqrt(sum(freq_dict_auteur[ngrammeinc]**2 for ngrammeinc in freq_dict_auteur))
            sim = dot_product / (norm_auteur * norm_inconnu)

            resultats.append((auteur, sim))
    ...
    def analyze(self):
        ...
        #Enlever les signes de ponctuation dans le texte selon PONC
        self.set_ponc(False)
        ...
        for auteur in self.auteurs:
            ...
            for oeuvre in aut_files:
                with open(oeuvre, 'r', encoding='utf8') as file:
                    ...
                    for i in range(len(words)-self.ngram + 1):
                        ngramme = " ".join(words[i:i+self.ngram])
